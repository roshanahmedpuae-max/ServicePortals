...
/**
 * ServiceOrderForm - Printers UAE Service Form
 * 
 * This is a separate, standalone form component for Printers UAE service orders.
...
export default function ServiceOrderForm({ onBack, businessUnit, prefilledData }: ServiceOrderFormProps) {
  ...
  useEffect(() => {
    if (prefilledData && employees.length > 0) {
      ...
      const serviceTypeName = serviceType?.name || "Service and Repair";
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="divide-y divide-gray-100 p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <FormSection
        ...
        subtitle="Describe the service request and work details"
        icon={
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        <Textarea
          ...
          placeholder="Describe the customer's request, problem description, and any complaints or issue notes..."
          rows={5}
          required
      ...
      <div className="p-4 sm:p-6 bg-gray-50 mobile-sticky-footer">
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-4 sm:justify-end">
          ...
          <Button
            ...
            fullWidth
            className="sm:w-auto sm:order-2 pulse-button"
            icon={
              !isSubmitting && (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
